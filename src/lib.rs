...
impl gl::Gl {
    unsafe fn CompileShaderExt(&self, shader: u32) -> Result<(), String> {
        self.CompileShader(shader);
        if return_param(|x| self.GetShaderiv(shader, gl::COMPILE_STATUS, x)) == gl::FALSE as _ {
            let log_length = return_param(|x| self.GetShaderiv(shader, gl::INFO_LOG_LENGTH, x));
            let mut _log_length = 0;
            if log_length > 0 {
                let mut buffer = Vec::with_capacity((log_length + 1) as usize);
                let ptr = buffer.as_mut_ptr();
                self.GetShaderInfoLog(shader, log_length, &mut _log_length, ptr);
                let log = std::ffi::CStr::from_ptr(ptr);
                Err(log.to_str().unwrap().to_owned())
            } else {
                Err("".to_owned())
            }
        } else {
...
impl Renderer {
    ...
    F: FnMut(&'static str) -> *const ::std::os::raw::c_void
  {
    ...
    unsafe {
      let glsl_version: &str = if cfg!(target_arch = "wasm32") {
            "#version 300 es\n\0"
      ...
      gl.CompileShaderExt(vert_shader)
          .expect("Could not compile the vertex shader!");
      gl.CompileShaderExt(frag_shader)
          .expect("Could not compile the fragment shader!");
